...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.theme_btn = QPushButton(self.tr("☀️ ライトモード"))
    ...
    def _toggle_theme(self):
        # 기존 애니메이션이 진행 중이면 중단하고 이전 오버레이 즉시 제거
        ...
        if hasattr(self, '_theme_overlay') and self._theme_overlay is not None:
            try:
                ...
                pass  # C++ 객체가 이미 삭제된 경우 (애니메이션 완료 시 자동 삭제됨)
            self._theme_overlay = None

        # 화면 페이드 아웃을 위한 오버레이 화면 캡처 생성
        ...
        self.theme_btn.setText(self.tr("☀️ ライトモード") if self.is_dark else self.tr("🌙 ダークモード"))
        self._sync_theme()
    ...
    def _update_network_ui(self, is_online):
        ...
        if is_online:
            ...
            self.network_lbl.setText(self.tr("🔴 オフライン"))
            self.network_lbl.setStyleSheet("color: #ff5252; font-weight: bold; padding: 5px;")
            logger.warning(self.tr("ネットワーク接続が切断されました。自動更新を一時停止します。"))

    def _sync_theme(self):
        """현재 설정된 테마(self.is_dark)를 모든 하위 위젯에 강제 적용합니다."""
    ...
    def _setup_tray_icon(self):
        ...
        self.tray_icon.setToolTip(self.tr("LEE電力モニター - バックグラウンド実行中"))
        
        tray_menu = QMenu()
        show_action = QAction(self.tr("開く (Open)"), self)
        show_action.triggered.connect(self._show_normal)
        
        quit_action = QAction(self.tr("完全に終了 (Quit)"), self)
    ...
    def closeEvent(self, event):
        ...
        msg_box.setWindowTitle(self.tr("終了の確認"))
        msg_box.setText(self.tr("アプリケーションを完全に終了しますか？\nそれともトレイ（バックグラウンド）に最小化しますか？"))
        
        btn_tray = msg_box.addButton(self.tr("トレイに最小化"), QMessageBox.ActionRole)
        btn_quit = msg_box.addButton(self.tr("完全に終了"), QMessageBox.DestructiveRole)
        btn_cancel = msg_box.addButton(self.tr("キャンセル"), QMessageBox.RejectRole)
        ...
        if msg_box.clickedButton() == btn_quit:
            self._quit_app()
            event.accept()
        elif msg_box.clickedButton() == btn_tray:
            event.ignore()
            self.hide()
            self.tray_icon.showMessage(
                self.tr("LEE電力モニター"),
                self.tr("バックグラウンドで実行中です。\nアイコンをダブルクリックで開きます。"),
